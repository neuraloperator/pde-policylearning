...
wandb.init(
    ...
    config={
        "model": "FNO2d rollout",
        "file_name": path_name,
        'python file': 'fourier_2d_rollout.py',
        "has_prev_press": True,
        "patches": False,
        "permute": False,
...
# QUESTION: Should I do the normalizer before I add a bunch of zeros? Does it make a difference?
...
train_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(x_train, y_train), batch_size=batch_size, shuffle=False, drop_last=True)
...
for ep in range(epochs):
    ...
    with torch.no_grad():
        ...
        for x, y in test_loader:
            print("x.shape:", x.shape)
            print("y_prev_test.shape:", y_prev_test.shape)
            # x[:, :, 1] = y_prev_test
            print("x.shape:", x.shape)
            x, y = x.cuda(), y.cuda()

            out = model(x, y_prev_test).reshape(batch_size, s1, s2)
            out = y_normalizer.decode(out)

...
wandb.init(
    ...
    config={
        ...
        "file_name": path_name,
        'python_file': 'fourier_rollout_eval.py',
        "has_prev_press": True,
        "patches": False,
        "permute": False,
...
model = torch.load("/central/groups/tensorlab/khassibi/fourier_neural_operator/outputs/min2_rollout_ep99")
...
#         torch.save(model, "/central/groups/tensorlab/khassibi/fourier_neural_operator/outputs/min2_rollout_ep"+str(ep))

T = 1000
input = torch.stack([P_test[0].reshape(s1,s2), V_test[0].reshape(s1,s2)]).reshape(1,s1,s2,2)
pred = torch.zeros(T,s1,s2)
truth = y_test[:T].reshape(T,s1,s2)
for t in range(T):
    input = x_normalizer.encode(input).cuda()
    print("input.shape:", input.shape)
    out = model(input).reshape(s1, s2)
    out = y_normalizer.decode(out)

    print("out.shape:", out.shape)
    # pred[t] = out.reshape(s1,s2)
    pred[t] = out
    input = torch.stack([P_test[t+1].reshape(s1,s2), out.cpu().reshape(s1,s2)]).reshape(1,s1,s2,2)
    # loss = myloss(out.view(batch_size,-1), y_test[t].view(batch_size,-1))
    # loss = myloss(out.cpu(), y_test[t]).item()
    loss = myloss(out.cpu().view(1,-1), y_test[t].view(1,-1)).item()
    print(t, loss)

scipy.io.savemat(output_path, mdict={'pred': out.cpu().numpy(), 'truth': truth.cpu().numpy()})
...
for index in [0, 5, 10, 19]:
    vmin = dat['truth'][index, :, :].min()
    vmax = dat['truth'][index, :, :].max()
    fig, axes = plt.subplots(nrows=1, ncols=3)
    plt.subplot(1, 2, 1)
    im2 = plt.imshow(dat['truth'][index, :, :], cmap='jet', aspect='auto', vmin=vmin, vmax=vmax)

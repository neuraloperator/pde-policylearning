...
class KFDataset(Dataset):
    ...
    def load(self):
        ...
        self.data = data.permute(0, 2, 3, 1)  # [N, S, S, T]
    ...
    def partition(self, data):
        ...
        Returns:
            output: int(1/t_duration) *N x (T//2 + 1) x S x S
        '''
        N, T, S = data.shape[:3]
        K = int(1 / self.t_duration)
    ...
    def __getitem__(self, idx):
        a_data = torch.cat((
            self.grid, 
            self.a_data[idx].repeat(1, 1, self.T, 1) # [S, S, T, 1]
        ), dim=-1)
        # self.data[idx]: [S, S, T]
        # a_data: [S, S, 33, 4]
...
'''
Support multiple reynolds
'''

class MultipleReynoldsKFaDataset(Dataset):
    def __init__(self, paths, 
                 data_res, pde_res, 
                 raw_res, 
                 n_samples=None, 
    ...
    def load(self):
        ...
        if self.t_duration != 0.:
            ...
            K = int(1/self.t_duration)
            step = end_t // K
            data, self.re = self.partition(data)
            a_data = raw_data[self.offset: self.offset + self.n_samples, 0:end_t:step, ::a_sub_x, ::a_sub_x]
            a_data = a_data.reshape(self.n_samples * K, 1, self.pde_res[0], self.pde_res[1])    # 2N x 1 x S x S
        else:
            a_data = raw_data[self.offset: self.offset + self.n_samples, 0:1, ::a_sub_x, ::a_sub_x]
    ...
    def __getitem__(self, idx):
        ...
        # self.data[idx]: [S, S, T]
        # a_data: [S, S, T, 4]
        return self.data[idx], a_data, self.re[idx]

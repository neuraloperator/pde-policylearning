from libs.pino_utils.losses import LpLoss
import torch
...
def Channelflow_PINO_loss(model_output, u0, forcing, v=1/40, t_interval=1.0):
    batchsize = model_output.size(0)
    nx = model_output.size(1)
    ny = model_output.size(2)
    nt = model_output.size(3)

    model_output = model_output.reshape(batchsize, nx, ny, nt)
    lploss = LpLoss(size_average=True)

    u_in = model_output[:, :, :, 0]
    loss_ic = lploss(u_in, u0)

    Du = FDM_NS_vorticity(model_output, v, t_interval)

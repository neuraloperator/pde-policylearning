...
class NormalizerGivenMeanStd(object):
    def __init__(self, mean, std, eps=0.00001, time_last=True):
        ...
        self.std_cuda = torch.tensor(std).cuda()
        self.eps = eps
    ...
    def cuda_decode(self, x, sample_idx=None):
        # sample_idx is the spatial sampling mask
        if sample_idx is None:
            std_cuda = self.std_cuda + self.eps # n
            mean_cuda = self.mean_cuda
        else:
            if self.mean_cuda.ndim == sample_idx.ndim or self.time_last:
                std_cuda = self.std_cuda[sample_idx] + self.eps  # batch*n
                mean_cuda = self.mean_cuda[sample_idx]
            if self.mean_cuda.ndim > sample_idx.ndim and not self.time_last:
                std_cuda = self.std_cuda[...,sample_idx] + self.eps # T*batch*n
                mean_cuda = self.mean_cuda[...,sample_idx]
        # x is in shape of batch*(spatial discretization size) or T*batch*(spatial discretization size)
        x = (x * std_cuda) + mean_cuda
        return x

...
class Identity(nn.Module):
    ...
    def __init__(self, in_features=None, out_features=None,
        ...
        # in_features = 1 + 2 * num_freqs * self.in_features
...
class FourierEmb(nn.Module):
    ...
    def __init__(self, in_features=None, out_features=None,
        ...
        self.fourier_feat_length = 16
        self.nerf_pos = NeRFPosEmbedding(num_freqs=self.fourier_feat_length)
        self.out_features = self.fourier_feat_length * 2 + 1
        # if in_features is not None and out_features is not None:
        #     self.id = nn.Linear(in_shape, out_features)
        # else:
        #     self.id = nn.Identity()

    def forward(self, x, edge=None, grid=None):
        pos_emb = self.nerf_pos(x)
        return pos_emb
...
def att_complex_matmul_1d(a, b):
    # (batch, in_channel, x), (in_channel, out_channel, x) -> (batch, out_channel, x)
    op = partial(torch.einsum, "bhif,bhfj->bhij")
    return torch.stack([
...
def freq_attention(query, key, value,
              mask=None, dropout=None, weight=None,
              attention_type='softmax', modes=16):

    seq_len = query.shape[2]
    query_ft = fft.rfft(query.permute(0, 1, 3, 2), n=seq_len, norm="ortho")
    query_ft = torch.stack([query_ft.real, query_ft.imag], dim=-1)
    
    key_ft = fft.rfft(key.permute(0, 1, 3, 2), n=seq_len, norm="ortho")
    key_ft = torch.stack([key_ft.real, key_ft.imag], dim=-1)
    
    value_ft = fft.rfft(value.permute(0, 1, 3, 2), n=seq_len, norm="ortho")
    value_ft = torch.stack([value_ft.real, value_ft.imag], dim=-1)
    d_k = query.size(-1)

    # switch time with feat dimension
    query_ft = query_ft[:, :, :, :modes, :].permute(0, 1, 3, 2, 4)
    key_ft = key_ft[:, :, :, :modes, :].permute(0, 1, 3, 2, 4)
    value_ft = value_ft[:, :, :, :modes, :].permute(0, 1, 3, 2, 4)
   
    if attention_type == 'cosine':
        raise RuntimeError("Not implemented")
    else:
        scores = att_complex_matmul_1d(query_ft, key_ft.transpose(-3, -2))
    ...
    out = att_complex_matmul_1d(p_attn, value_ft)
    out_ft = torch.complex(out[..., 0], out[..., 1])
...
def full_attention_conv(qs, ks, vs, kernel, output_attn=False):
    ...
    if kernel == 'simple':
        # normalize input
        qs = qs / torch.norm(qs, p=2) # [B, H, N, M]
        ks = ks / torch.norm(ks, p=2) # [B, H, L, M]
        bs, N = qs.shape[0], qs.shape[2]

        # numerator
        kvs = torch.einsum("bhlm,bhld->bhmd", ks, vs)
        attention_num = torch.einsum("bhnm,bhmd->bnhd", qs, kvs) # [BS, N, H, D]
        bs, head, seq_len, hidden = vs.shape
        all_ones = torch.ones([bs, seq_len]).to(vs.device)
        vs_sum = torch.einsum("bl,bhld->bhd", all_ones, vs) # [BS, H, D]
        attention_num += vs_sum.unsqueeze(1).repeat(1, N, 1, 1) # [B, N, H, D]

        # denominator
        bs, head, seq_len, hidden = ks.shape
        all_ones = torch.ones([bs, seq_len]).to(ks.device)
        ks_sum = torch.einsum("bhlm,bl->bhm", ks, all_ones)
        attention_normalizer = torch.einsum("bhnm,bhm->bnh", qs, ks_sum)  # [B, N, H]
        ...
        if output_attn:
            attention = torch.einsum("bhnm,bhlm->bnlh", qs, ks) / attention_normalizer # [B, N, L, H]
    ...
    def forward(self, query, key, value, pos=None, mask=None, weight=None):
        ...
        query, key, value = \
            [layer(x).view(bsz, -1, self.n_head, self.d_k).transpose(1, 2)
             for layer, x in zip(self.spectral_conv, (query, key, value))]
        # query, key, value = \
        #     [layer(x).view(bsz, -1, self.n_head, self.d_k).transpose(1, 2)
...
class FourierSpectralConv1d(nn.Module):
    def __init__(self, in_dim,
                 ...
                 debug=False, 
                 band_num=5):
        super(FourierSpectralConv1d, self).__init__()
        '''
        Wrap several Fourier Spectral Conv1d modules with Fourier feature embddings
        '''
        self.band_num = band_num
        self.return_freq = return_freq
        self.spec_convs = nn.ModuleList([SpectralConv1d(in_dim=in_dim, out_dim=out_dim, modes=modes, n_grid=n_grid,
                                       dropout=dropout, return_freq=return_freq, activation=activation,
                                       debug=debug) for i in range(band_num)])
    ...
    def forward(self, x):
        ...
        for i in range(self.band_num):
            if (i == 0):
                ...
                cur_input = torch.sin((2 ** i) * x)
            else:
                cur_input = torch.cos((2 ** i) * x)
            
            if self.return_freq:
                out_x, out_ft = self.spec_convs[i](cur_input)
                all_freq += out_ft
            else:
                out_x = self.spec_convs[i](cur_input)
            all_x += out_x
        res_x = all_x / self.band_num
        res_freq = all_freq / self.band_num
        if self.return_freq:
            return res_x, res_freq         
        else:
            return res_x

from libs.models.attention_layers import *
from libs.models.utils_ft import *
import copy
import os
import sys
import pdb
...
class SimpleTransformerEncoderLayer(nn.Module):
    ...
    def forward(self, x, pos=None, weight=None):
        ...
        ori_x = x
        if self.add_pos_emb:
            pos_x = x.permute((1, 0, 2))
            pos_x = self.pos_emb(pos_x)
...
class SimpleTransformer(nn.Module):
    ...
    def forward(self, node, v_plane, pos=None, grid=None, weight=None):
        x_latent = []
        attn_weights = []
        batch_size, timestep, width, height, dim = node.shape
        node = node.reshape(batch_size, -1, dim)
        # x: [8 BS, 2048 T, 1 D], edge: None
        x = self.feat_extract(node, None)
        if self.spacial_residual or self.return_latent:
            res = x.contiguous()
            x_latent.append(res)

        # x: [8, 2048, 96], pos: [8, 2048, 1], weight: None
        ...
        x = x.reshape(batch_size, timestep, width, height, dim)
        # x: [8, 2048, 1], x_freq: None, x_latent: [], attn_weights: []
        return x
    ...
    def _get_setting(self):
        ...
        self.attention_types = ['fourier', 'integral',
                                'cosine', 'galerkin', 'linear', 'softmax']  # used fourier
        
        # overwrite settings below
        self.num_encoder_layers = 8
        self.num_regressor_layers = 3
    ...
    def _get_feature(self):
        if self.num_feat_layers > 0 and self.feat_extract_type == 'gcn':
            ...
            self.feat_extract = Identity(in_features=self.node_feats,
                                         out_features=self.n_hidden)
        self.use_difformer = False # modified by Zelin
    ...
    def _get_regressor(self):
        if self.decoder_type == 'pointwise':
            raise NotImplementedError("Deprecated point wise regressor!")
